...
/**
 * Call LLM API with streaming. Returns { content, toolCalls, error }
 */
function callLLM(messages, tools, apiKey, model, onToken) {
  const isAnthropic = AI_API_BASE.includes('anthropic');
  if (isAnthropic) {
    return callAnthropic(messages, tools, apiKey, model, onToken);
  }
  return callOpenAICompatible(messages, tools, apiKey, model, onToken);
}
...
module.exports = { runAgent, buildMessages, parseTextToolCalls, callLLM };
